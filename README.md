...
- Создание проекта: в селекте «Действия» можно добавить пустой проект (указать ID и имя) для дальнейших переносов чатов.
- Перенос чатов: кнопка ↕ позволяет перенести чат в другой проект того же аккаунта (обновляются файлы и индекс, сохраняется override для следующего импорта). Если исходный проект опустел, его папка удаляется и пометка сохраняется в `project_overrides.json`, чтобы при переимпорте сохранялась новая структура.
